...
<?php 
    ...
    if($current_code_hash == null || $current_page_hash == null) {
        ...
        $verify = 1;
        $sql = "SELECT * FROM user_vn WHERE user_name='$username' AND verify='$verify'";
      ...
      <div class="box-root padding-top--24 flex-flex flex-direction--column" style="flex-grow: 1; z-index: 9;">
        ...
        <?php 
            ...
            if( isset($_POST['submit']) && $_POST["verify"] != '') {
                $username = $current_page_hash;
                $verify = $_POST["verify"];
                $sql = "SELECT * FROM user_vn WHERE user_name='$username' AND verify_code='$verify'";
                $user = mysqli_query($conn,$sql);
                $result = $conn->query($sql);
                if(mysqli_num_rows($user)>0) {
                    while($row = $result->fetch_assoc()) {
                    ...
                    $_SESSION["thongbao"] = "Bạn đã đăng nhập sai code";
                    //header("location:login.php"); 
                }
            } else {
