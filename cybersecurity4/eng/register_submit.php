<?php
    ...
    if( isset($_POST['submit']) && $_POST["username"] != '' && $_POST["password"] != '' && $_POST["repassword"] != '' && $_POST["email"] != '') {
        ...
        if ($password != $repassword ){
            $_SESSION["thongbao"] = "Password re-entered is incorrect";
        ...
        if (mysqli_num_rows($old)>0) {
            $_SESSION["thongbao"] = "Username already exists !!! Please use another username";
            header("location:index.php");
            die();
        }
    ...
    //     }
        ...
        $sql = "INSERT INTO user_eng (user_name,email,password,verify_code,verify,level) VALUES ('$username','$email','$password','$verificationCode',1,'$level')";
        mysqli_query($conn,$sql);
        ?> <?php
        $_SESSION["thongbao"] = "Registered sussesfully, please login to your account!";
        header("location:login.php");
        ?><?php

        
    } else {
        $_SESSION["thongbao"] = "Please enter the full requirement field";
        header("location:index.php");
    }
?>