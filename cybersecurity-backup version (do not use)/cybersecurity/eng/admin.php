    ...
    <div style="background-color: rgb(219, 226, 245); padding-top: 3%; padding-bottom:10%">
      <div>
        ...
        <h2>SUMMARY TABLE OF PLAYERS</h2>
         <?php
        include 'config.php';
        $sql_question = "SELECT * FROM user_eng";

        $sql_count = "SELECT * FROM user_eng WHERE role='user'";
        $sql_count_hooked = "SELECT * FROM user_eng WHERE hooking=1 AND role='user'";
        $sql_count_top3 = "SELECT user_name,phishing_result FROM user_eng WHERE role='user' order by phishing_result DESC LIMIT 3 ";
        ...
        <?php 
         include 'config.php';
        $sql_question = "SELECT SUM(login_count) FROM user_eng";
